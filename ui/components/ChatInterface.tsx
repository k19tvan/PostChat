...
export const ChatInterface: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full w-full relative overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
      {/* Enhanced decorative background elements */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-purple-600/8 rounded-full blur-3xl translate-x-1/4 -translate-y-1/4" />
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-indigo-600/8 rounded-full blur-3xl -translate-x-1/4 translate-y-1/4" />
        ...
        <div className="max-w-4xl mx-auto space-y-6">
            ...
            >
              ...
              `}>
                <div className={`
                  ...
                  ${msg.role === 'user'
                    ? 'bg-gradient-to-br from-indigo-600 to-indigo-700 text-white shadow-indigo-900/40 rounded-tr-md'
                    : msg.isError
                      ? 'bg-red-950/40 border border-red-500/30 text-red-200 backdrop-blur-md'
                      : 'bg-slate-800/60 text-slate-100 border border-slate-700/50 shadow-black/30 backdrop-blur-md rounded-tl-md'
                  }
                `}>
                ...
                <div className={`
                  ...
                  ${msg.role === 'user' ? 'text-slate-500' : 'text-slate-600'}
                `}>
                  ...
                  <div className="mt-2 space-y-1.5">
                    ...
                    <div className="space-y-1">
                      {msg.sources.slice(0, 3).map((source, idx) => (
                        <div
                          key={idx}
                          className="text-[11px] px-3 py-2 bg-slate-900/60 border border-slate-700/30 rounded-lg hover:border-violet-500/30 transition-colors"
                        >
                          <div className="flex items-start justify-between gap-2">
                            <p className="text-slate-400 line-clamp-2 flex-1">
                              {source.content}
                            </p>
                            <span className="text-[9px] text-violet-400 font-mono shrink-0">
          ...
          {isLoading && (
            <div className="flex items-start gap-3 sm:gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-gradient-to-br from-violet-600 to-purple-600 flex items-center justify-center shadow-lg shadow-violet-500/30">
                <Bot size={20} className="text-white" />
              </div>
              <div className="bg-slate-800/60 backdrop-blur-md px-5 py-4 rounded-2xl rounded-tl-md border border-slate-700/50 shadow-xl flex items-center gap-3">
      ...
      <div className="flex-none p-4 sm:p-6 bg-gradient-to-t from-slate-950 via-slate-950/95 to-transparent relative z-20 border-t border-slate-800/50">
        <form
        ...
        >
          <div className="relative flex items-center">
            <input
              ...
              className="w-full bg-slate-800/60 backdrop-blur-xl text-slate-100 placeholder-slate-500 rounded-2xl pl-5 sm:pl-6 pr-14 sm:pr-16 py-4 sm:py-5 border border-slate-700/50 focus:outline-none focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/20 transition-all shadow-xl shadow-black/30 hover:border-slate-600/50 disabled:opacity-50 disabled:cursor-not-allowed text-[15px] sm:text-base"
              disabled={isLoading}
            />
            <button
              type="submit"
